...
const textFromImage = require('./togetherai.js');
...
export async function getItems(receiptStr) {
    ...
    const data = {
        ...
        messages: [{
            role: "user",
            content: `Return the names of all food items from the following text extracted from a Grocery Bill: '${receiptStr}' Return these items in the form of a python list. Make sure to only include food items. Also strictly only return the python list with food items. Do not return anything else. Just the python list. Your response must only contain the python list and no other words.`
        }]
    };
...
export async function getRecipe(foodItems) {
    const url = `${baseUrl}/v1/chat/completions`;
    const headers = {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${apiKey}`
...
export async function getTextFromImage(inputFilePath) {
...
export function removeUnnecessaryLines(receiptString) {
...
export async function processReceipt(file_path) {
    const text =  getTextFromImage(file_path);
...
processReceipt('/Users/sravankumargorati/Downloads/so-we-went-to-trader-joes-and-the-receipt-had-tax-on-the-v0-8u9il0scnjda1.webp');